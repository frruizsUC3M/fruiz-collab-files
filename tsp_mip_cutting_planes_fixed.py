...
class TSP(object):
    ...
    def load_problem(self, graph, free_edges):
    ...
    def add_variables(self):
        ...
        self.x = [[self.model.add_var(var_type=BINARY) for j in self.V] for i in self.V]

        # continuous variable to prevent subtours: each city will have a
        # different sequential id in the planned route except the first one
        self.y = [self.model.add_var() for i in self.V]

    def add_fixed_edge_constraints(self):

        for u, v in self.Arcs:
            if not (u, v) in self.free_edges.edges(u):
                self.model += self.x[u][v] == 0
    ...
    def add_constraints(self):
        ...
        self.add_fixed_edge_constraints()
    ...
    def build(self, min_obj_value=None, max_n_solutions=None):
        ...
        self.F, G = [], nx.DiGraph()
        for (i, j) in self.Arcs:
            G.add_edge(i, j, weight=self.graph[i][j]['weight'])
        for i in self.V:
            P, D = nx.dijkstra_predecessor_and_distance(G, source=i)
            DS = list(D.items())
            DS.sort(key=lambda x: x[1])
            self.F.append((i, DS[-1][0]))

        self.model.cuts_generator = SubTourCutGenerator(self.F, self.x, self.V)
